...
The limit on this is the **[smart zone](https://www.aihero.dev/ai-coding-dictionary/smart-zone)**: the portion of the context window within which the model still reasons sharply. If a session's quality is visibly degrading before `/to-tickets`, don't push on — `/compact` at the nearest phase boundary and carry on (see Phase boundaries).
