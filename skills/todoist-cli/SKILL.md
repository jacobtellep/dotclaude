...
**Default due date — set it to today.** Unless the user specifies a due date (or explicitly asks for an undated / "someday" task), create every task with a due date of **today**: `td task add "..." --due today`, or include `today` in a `td task quickadd` string. Rationale: Todoist Inbox/Today views are commonly filtered by date, so an undated task can be invisible to the user even though it was created successfully — defaulting to today guarantees it surfaces. Only omit `--due` when the user clearly wants no date.
